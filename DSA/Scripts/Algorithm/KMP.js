...
function createblocks(word,pattern) {
    ...
    for (let i = 0 ; i < pattern.length ; i++)  {
        ...
        if (pattern[i] == " ")  patterncontainer.insertAdjacentHTML("beforeend" , `<div id=pattern${i} class="charpatterns"  style="background-color: #8ab6d6;transition: ${speed}ms linear;"><p>&#8203;</p><p style="margin:0;margin-top:-80px;left:-80px;color:black;font-size:80%;">${i}</p></div>`)

        else patterncontainer.insertAdjacentHTML("beforeend" , `<div id=pattern${i} class="charpatterns"  style="background-color: #8ab6d6;transition: ${speed}ms linear;"><p>${pattern[i]}</p><p style="margin:0;margin-top:-80px;left:-80px;color:black;font-size:80%;">${i}</p></div>`)


        knuthvalues.insertAdjacentHTML("beforeend" , `<div id=knuth${i} class="charpatterns"  style="background-color: #8ab6d6;transition: 100ms linear;"><p id=KMP${i}>&#8203;</p></div>`)
...
async function computeLPSArray( pat,  M)
{
    ...
    for (ie = 0; ie < x.length; ie++) {
      x[ie].style.backgroundColor = "#8ab6d6";
    }
