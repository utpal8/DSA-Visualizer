...
document.getElementById("svg1").insertAdjacentHTML("beforeend", `<rect class="bars" id="bigr${i}" x="${15+(i*5)}" y="${((maxheight+35)-(ra/n)*maxheight)}" width="4" height="${(ra/n)*maxheight}"style="fill:#8ab6d6;stroke:black;stroke-width:.3;" />`)
...
function mergerinsert(i , type , j)  {
  ...
  document.getElementById("bigr"+type+i).style.fill = "#8ab6d6"
...
async function swapper(i1,i2)  {
 ...
 e1.style.fill = '#8ab6d6';
 e2.style.fill = '#8ab6d6';
   ...
   // recurse(low , high)
    ...
    for (var qj = low; qj <= high - 1; qj++)
    {
        ...
        // await hilight("aitem"+qj,defaultcolor)
            ...
            document.getElementById("bigr"+high).style.fill = "#8ab6d6";
    return (qi + 1);
}
...
{
    // Start with a big gap, then #8ab6d6uce the gap
...
async function HeapSort()
{
    ...
    for (let i = n - 1; i > 0; i--) {
      ...
      document.getElementById("bigr"+i).style.opacity = "20%";
        ...
        // call max heapify on the #8ab6d6uced heap
        await   heapify( i, 0);
    }
